...
def seed_timewise(p: int = 1, q: int = 2, threshold: float = 1.0 / 20.0) -> None:
    """
    Copy the base predictor weights into time-wise partial files for powers of (p/q):
    ...
    out_dir = os.path.join(BASE_DIR, "NN_weights", "game_prediction", "partial_state_prediction_time-wise")
    os.makedirs(out_dir, exist_ok=True)

    # Helper to copy from a given source weights file
    def copy_from(src_path: str, num: int, den: int) -> str:
        dst_path = os.path.join(out_dir, f"state_predictor_partial_time-wise_{num}_{den}.pt")
    ...
    num, den = 1, 1
    path_1_1 = os.path.join(out_dir, f"state_predictor_partial_time-wise_{num}_{den}.pt")
    ...
    while value > threshold:
        ...
        if os.path.exists(dst_path):
            # Still advance the source so next power copies from the latest file
